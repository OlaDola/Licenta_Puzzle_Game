...
public class Grid<TGridObject>
{
    ...
    public Grid(int width, float cellSize, Vector3 originPosition, Func<Grid<TGridObject>, int, int,TGridObject> createGridObject)
    {
        ...
        for (int i = 0; i < gridArray.GetLength(0); i++)
        {
            for (int j = 0; j < gridArray.GetLength(1); j++)
            {
                debugTextArray[i, j] = Utils_Class.CreateWorldText(gridArray[i, j]?.ToString(), 
                    GameObject.Find("PositionLocation").transform, 
                    GetWorldPosition(i, j) + new Vector3(cellSize, 0, cellSize) * .5f, 
                    8, Color.black, TextAnchor.MiddleCenter);
            }
        }
