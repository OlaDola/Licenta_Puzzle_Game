...
public class Grid_Show : MonoBehaviour
{
    ...
    private IEnumerator LoadFunction(string levelName)
    {
        ...
        if(isPlayMode == 1)
        {
            Vector3 StartPosition = grid.GetWorldPosition(0, 0) + new Vector3(0.5f, 0, 0.5f) * gridCellSize + new Vector3(0,2,0);
            Instantiate(Player.transform, StartPosition, Quaternion.identity);
        }
    ...
    public void Delete(string levelToDelete)
    {
        SaveSystem.Delete(levelToDelete);
    }
}
